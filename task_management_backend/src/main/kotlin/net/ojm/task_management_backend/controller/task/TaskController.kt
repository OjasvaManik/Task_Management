...
class TaskController(
    ...
    fun createTask(@RequestBody request: CreateTaskRequest): ResponseEntity<CreateTaskResponse> {
        return ResponseEntity.ok(taskService.createTask(request))
    }

    @GetMapping
    fun getAllTasksByGroup(@RequestBody request: GetTasksRequest): ResponseEntity<List<GetTasksResponse>> {
        return ResponseEntity.ok(taskService.getAllTasksByGroup(request))
    }
